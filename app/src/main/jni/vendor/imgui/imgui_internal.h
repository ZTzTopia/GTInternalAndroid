...
struct ImGuiContext;                // Main Dear Gui context
...
//-----------------------------------------------------------------------------
// [SECTION] Generic helpers
// Note that the ImXXX helpers functions are lower-level than Gui functions.
// Gui functions or the Gui context are never called/used from other ImXXX functions.
//-----------------------------------------------------------------------------
// - Helpers: Hashing
// - Helpers: Sorting
...
// You may want to create your own instance of this if you want to use ImDrawList completely without Gui. In that case, watch out for future changes to this structure.
...
namespace ImGui
{
    ...
    // - You are calling Gui functions after Gui::EndFrame()/Gui::Render() and before the next Gui::NewFrame(), which is also illegal.
...
//#endif
    ...
    // Basic Accessors
    inline ImGuiID          GetItemID()     { ImGuiContext& g = *GImGui; return g.LastItemData.ID; }   // Get ID of last item (~~ often same Gui::GetID(label) beforehand)
